...
func setup(): # TODO: get thr changin coloring for visited nodes to work
	...
	for i in range(2 * DATABASE.mapSize.x + 1):
		for j in range(2 * DATABASE.mapSize.y + 1):
			var c = ColorRect.new()
			c.size = tileIconSize
			c.position = center + (i - DATABASE.mapSize.x) * tileIconSize * Vector2(1,0)  + (j - DATABASE.mapSize.y) * tileIconSize * Vector2(0,1)
			var colorVals =  DATABASE.BIOME_GRID[i][j]
